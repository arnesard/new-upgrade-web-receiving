                        ...
                        <form action="{{ route('overtime.store') }}" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="reason" class="form-control form-control-custom" rows="4"
                                    placeholder="Contoh: Target produksi belum tercapai..." required></textarea>
                            </div>
                            ...
                            @forelse($overtimes as $ot)
                                ...
                                <div class="overtime-record p-4 mb-3 print-no-margin print-border-bottom">
                                    <div class="d-flex justify-content-between align-items-start mb-2">
                                        <div>
                                            <div class="d-flex align-items-center gap-2 flex-wrap">
                                                <p
                                                    class="fw-bold text-slate-900 mb-0 {{ !$isApproved ? 'd-print-none' : '' }}">
                                                    {{ $ot->employee_name }}</p>
                                                @if (!$isApproved)
                                                    <p class="fw-bold text-muted mb-0 d-none d-print-block"><em>[Nama
                                                            Tersembunyi - Belum Disetujui]</em></p>
                                                @endif
                                                @if ($displayId)
                                                    <span
                                                        class="badge bg-primary bg-opacity-10 text-primary border-0 rounded-pill text-xs fw-bold px-2 py-1 {{ !$isApproved ? 'd-print-none' : '' }}"
                                        ...
                                        <div class="text-end">
                                            ...
                                            <div class="text-xs text-muted">
                                                {{ \Carbon\Carbon::parse($ot->start_time)->format('H:i') }} -
                                                {{ \Carbon\Carbon::parse($ot->end_time)->format('H:i') }}</div>
                                            <span
                                                class="badge badge-status-{{ $ot->status }} rounded-pill px-2 py-1 text-xs fw-bold mt-1">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        <p class="text-xs text-slate-400 mb-0">Diajukan:
                                            {{ $ot->created_at->format('H:i') }}</p>
                                            ...
                                            @if (!auth()->user()->isLeader())
                                                @if ($ot->status == 'pending')
                                                    ...
                                                    <button type="button"
                                                        ...
                                                        onclick="openRejectModal({{ $ot->id }}, '{{ $ot->employee_name }}')">
            ...
            <div class="modal-content border-0 rounded-4">
                ...
                <form id="rejectForm" method="POST">
                    ...
                    <div class="modal-body p-4">
                        <p class="text-sm text-slate-600 mb-4">Alasan penolakan untuk <strong
                                id="rejectEmployeeName"></strong>:</p>
