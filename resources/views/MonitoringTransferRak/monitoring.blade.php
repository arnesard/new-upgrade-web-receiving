...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .app-title {
        font-size: 14px; font-weight: 800; color: #64c8ff;
    ...
    .operator-badge span { color: #64c8ff; font-weight: 600; max-width: 100px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .operator-badge:hover { border-color: rgba(59,130,246,0.5); }
    ...
    .info-pill-icon { font-size: 13px; }
    .info-pill-body { flex: 1; min-width: 0; }
    .info-pill-label { font-size: 9px; color: #64748b; text-transform: uppercase; letter-spacing: 0.4px; }
    .info-pill-value { font-size: 12px; font-weight: 600; color: #64c8ff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .info-pill-value.empty { color: #475569; }
    ...
    .mulai-footer.hidden { display: none; }
    ...
    .btn-start:not(:disabled) { animation: readyPulse 2s ease-in-out infinite; }
    .btn-start:disabled {
    ...
    .btn-start-hint {
        ...
        min-height: 14px; transition: all 0.2s;
    }
    @keyframes readyPulse {
        0%, 100% { box-shadow: 0 4px 20px rgba(59,130,246,0.3); }
        50% { box-shadow: 0 4px 30px rgba(59,130,246,0.6), 0 0 0 4px rgba(59,130,246,0.1); }
    }

    .setup-row { margin-bottom: 10px; }
    .setup-label { font-size: 10px; color: #64748b; text-transform: uppercase; letter-spacing: 0.4px; margin-bottom: 4px; display: block; }
    ...
    .setup-input:focus, .setup-select:focus { border-color: #60a5fa; background: rgba(59,130,246,0.13); }
    .setup-input::placeholder { color: #475569; }

    /* Datalist style hint */
    .input-hint { font-size: 10px; color: #475569; margin-top: 3px; }
    ...
    .scan-input-label { font-size: 10px; color: #64748b; text-transform: uppercase; letter-spacing: 0.4px; margin-bottom: 6px; display: block; }
    #scanInput {
    ...
    #scanInput:focus { border-color: #60a5fa; box-shadow: 0 0 0 3px rgba(59,130,246,0.2); }
    #scanInput:disabled { opacity: 0.35; cursor: not-allowed; }
    #scanInput::placeholder { color: #475569; font-weight: 400; letter-spacing: 0; }
    ...
    .counter-label { font-size: 10px; color: #64748b; text-transform: uppercase; }
    .counter-value { font-size: 42px; font-weight: 900; color: #64c8ff; text-shadow: 0 0 20px rgba(100,200,255,0.5); line-height: 1; }
    ...
    .scan-no { font-size: 11px; font-weight: 700; color: #64748b; width: 22px; }
    .scan-code { font-family: 'Courier New', monospace; font-size: 13px; font-weight: 600; color: #22c55e; flex: 1; text-align: center; }
    .scan-time { font-size: 10px; color: #475569; }
    ...
    .btn { flex: 1; padding: 12px; border-radius: 10px; border: none; font-weight: 700; font-size: 13px; cursor: pointer; transition: all 0.2s; }
    ...
    .btn-finish { background: linear-gradient(135deg, #22c55e, #16a34a); color: white; box-shadow: 0 4px 12px rgba(34,197,94,0.2); }
    .btn-finish:hover { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(34,197,94,0.35); }
    .btn-finish:disabled { opacity: 0.35; cursor: not-allowed; transform: none; }
    ...
    .toast.error { background: rgba(239,68,68,0.15); border: 1px solid rgba(239,68,68,0.4); color: #f87171; box-shadow: 0 8px 20px rgba(239,68,68,0.15); }
    .toast.success { background: rgba(34,197,94,0.15); border: 1px solid rgba(34,197,94,0.4); color: #4ade80; box-shadow: 0 8px 20px rgba(34,197,94,0.15); }
    ...
    .modal-title { font-size: 15px; font-weight: 700; color: #64c8ff; margin-bottom: 14px; text-align: center; }
    .modal-select { width: 100%; padding: 11px 12px; background: rgba(59,130,246,0.08); border: 1px solid rgba(59,130,246,0.25); border-radius: 8px; color: #e2e8f0; font-size: 14px; outline: none; margin-bottom: 12px; appearance: none; }
    .modal-btn { width: 100%; padding: 12px; background: linear-gradient(135deg, #3b82f6, #6366f1); border: none; border-radius: 8px; color: white; font-size: 14px; font-weight: 700; cursor: pointer; }
    ...
    .scan-list::-webkit-scrollbar-thumb { background: rgba(59,130,246,0.3); border-radius: 2px; }
    ...
    .center-actions.visible { display: flex; }
    ...
    .btn-center-cancel:active { background: rgba(239,68,68,0.25); transform: scale(0.97); }
    ...
    .btn-center-finish:not(:disabled):active { transform: scale(0.97); }
    .btn-center-finish:not(:disabled) { animation: finishPulse 2.5s ease-in-out infinite; }
...
<div class="app-wrap">
    ...
    <div class="setup-panel" id="setupPanel">
        ...
        <div class="setup-row">
            ...
            <input list="driverList" class="setup-input" id="supirInput" placeholder="Ketik atau pilih supir..." autocomplete="off">
        ...
        <div class="setup-row" style="margin-bottom:0">
            ...
            <input type="text" class="setup-input" id="mobilInput" placeholder="Arahkan scanner ke barcode kendaraan...">
        </div>
    </div>
    ...
    {{-- SCAN AREA --}}
    <div class="scan-area">
        <audio id="beepOk" preload="auto"><source src="{{ asset('sounds/WindowsDing.wav') }}" type="audio/wav"></audio>
...
document.getElementById('btnConfirmOperator').addEventListener('click', () => {
    ...
    if (!id) { showToast('Pilih operator dulu!', 'error'); return; }
...
mobilInput.addEventListener('keypress', (e) => {
    if (e.key === 'Enter') { mobilInput.blur(); checkStartReady(); }
});
...
document.getElementById('scanInput').addEventListener('keypress', async (e) => {
    ...
    if (!state.transferId) { showToast('Transfer belum dimulai!', 'error'); return; }

    try {
        const res = await fetch('/transfer-rak/scan', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': CSRF },
            body: JSON.stringify({ transfer_rak_id: state.transferId, kode_rak: kode })
        });
        const data = await res.json();
        ...
        state.scanList.push({ kode, waktu: data.waktu_scan });
...
document.getElementById('btnFinish').addEventListener('click', async () => {
    ...
    try {
        const res = await fetch('/transfer-rak/finish', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': CSRF },
            body: JSON.stringify({ transfer_rak_id: state.transferId })
        });
        const data = await res.json();
        if (!data.success) throw new Error(data.error);
